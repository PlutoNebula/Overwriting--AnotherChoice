...
(function (w) {
  ...
  var OW = (w.OW = w.OW || {});
  var D = w.document;
  var ORIGIN = (w.OW_API_BASE || D.documentElement.getAttribute('data-api') ||
    'http://127.0.0.1:8000').replace(/\/+$/, '');
  var BACKEND = ORIGIN + '/api/v1';

  function userId() {
    var state = (OW.Store && OW.Store.get && OW.Store.get()) || {};
    return state.reader || 'guest';
  }

  function sync(path, options) {
    options = options || {};
    return w.fetch(ORIGIN + path, options).then(function (res) {
      if (res.ok) return res.json().catch(function () { return {}; });
      return res.json().catch(function () { return {}; }).then(function (data) {
        throw new Error(data.detail || ('同步失败（' + res.status + '）'));
      });
    }).catch(function (err) {
      // 浏览器 localStorage 始终是主存储；后端不可用时不打断现场演示。
      if (w.console && w.console.warn) w.console.warn('[覆写同步]', err.message || err);
      return null;
    });
  }
  ...
  function branchForBackend(branch) {
    return {
      id: branch.id,
      parentId: branch.parentId || null,
      title: branch.title || '',
      narrative: branch.content || branch.narrative || '',
      changes: branch.keyChanges || branch.changes || [],
      conflicts: branch.conflicts || [],
      nextDirections: branch.nextDirections || branch.next_directions || [],
      origin: {
        ch: branch.chapterIndex || 0,
        para: branch.paragraphIndex || 0,
        mode: branch.sourceType || 'chapter_end',
        quote: branch.quote || ''
      },
      form: {
        prompt: branch.intent || '',
        tones: branch.tendencies || [],
        strength: branch.intensity || 'medium',
        constraints: branch.mustPreserve || ''
      },
      result: branch,
      status: branch.status || 'candidate',
      demo: !!branch.isDemo,
      editedByReader: !!branch.editedByReader,
      at: branch.createdAt || Date.now()
    };
  }
  ...
  OW.Api = {
    ...
    saveBook: function (book) {
      return sync('/api/books/' + encodeURIComponent(book.id) +
        '?user_id=' + encodeURIComponent(userId()), jsonOptions('PUT', book));
    },
    deleteBook: function (bookId) {
      return sync('/api/books/' + encodeURIComponent(bookId) +
        '?user_id=' + encodeURIComponent(userId()), { method: 'DELETE' });
    },
    saveBranch: function (bookId, branch) {
      return sync('/api/books/' + encodeURIComponent(bookId) + '/branches/' +
        encodeURIComponent(branch.id) + '?user_id=' + encodeURIComponent(userId()),
        jsonOptions('PUT', branchForBackend(branch)));
    },
    deleteBranch: function (bookId, branchId) {
      return sync('/api/books/' + encodeURIComponent(bookId) + '/branches/' +
        encodeURIComponent(branchId) + '?user_id=' + encodeURIComponent(userId()),
        { method: 'DELETE' });
    },
    saveInscription: function (bookId, inscription) {
      return sync('/api/books/' + encodeURIComponent(bookId) + '/inscriptions/' +
        encodeURIComponent(inscription.id) + '?user_id=' + encodeURIComponent(userId()),
        jsonOptions('PUT', inscription));
    },
    deleteInscription: function (bookId, inscriptionId) {
      return sync('/api/books/' + encodeURIComponent(bookId) + '/inscriptions/' +
        encodeURIComponent(inscriptionId) + '?user_id=' + encodeURIComponent(userId()),
        { method: 'DELETE' });
    }
  };
})(window);
